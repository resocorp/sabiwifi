...
def create_ticket(*, reseller, type, subject, body='', lead=None, subscriber=None,
    ...
    # Hand off unassigned support/install tickets to the field-supervisor AI.
    # Wrapped so a broken ai/ config never blocks a ticket create call.
    if not assigned_staff and type in (Ticket.TYPE_SUPPORT, Ticket.TYPE_INSTALL):
        try:
            if hasattr(reseller, 'ai_config'):
                from ai.jobs import enqueue_field_agent
                transaction.on_commit(lambda tid=ticket.pk: enqueue_field_agent(tid))
        except Exception:
            pass
    return ticket
